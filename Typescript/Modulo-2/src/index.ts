import type { Estudiante, Asignatura, EstadoMatricula } from "./domain/types/entities.js";
import { generarReporte } from "./domain/reporte.js";
import {
  obtenerRecurso,
  obtenerRecursoDirecto,
  type RespuestaAPI,
...
async function demostrarServicios() {
  ...
  const respuestaEstudiante = await obtenerRecurso<Estudiante>(
    "/estudiantes/1"
  );
  ...
  try {
    const asignatura = await obtenerRecursoDirecto<Asignatura>(
      "/asignaturas/101"
    );
  ...
  const respuestaInexistente = await obtenerRecurso<unknown>(
    "/estudiantes/999"
  );
  ...
  const respuestaEstudiante2 = await obtenerRecurso<Estudiante>(
    "/estudiantes/2"
  );
  ...
  if (respuestaEstudiante2.success && respuestaEstudiante2.data) {
    ...
    console.log(
      `Estudiante: ${nombre}, Carrera: ${carrera}, Semestre: ${semestre}`
    );
  }
